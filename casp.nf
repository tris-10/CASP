...
process ref_snv_call {
    ...
    if [ "\$(cut -f 1 ref_phase.gtf | uniq | wc -l)" -eq \$(cat ref_phase.gtf | wc -l) ]; then
        ...
        meth_phaser_post_processing -ib ref_tagged.bam -if methphaser_out/work -ov ref_meth_phase_temp.vcf -ob mp_tagged -vc ref_phase.vcf.gz -mc ${params.meth_min_reads} -vd ${params.meth_min_diff}
        ...
        python ${params.script_dir}/parse_methphaser_stats.py methphaser_out/work/${params.chrom}/*csv ref_meth_phase_stats.tsv --min_reads ${params.meth_min_reads} --min_percentage ${params.meth_min_diff}
    fi
...
process asm_initial {
    ...
    """
    canu maxMemory=24G maxThreads=${task.cpus} useGrid=false genomeSize=${params.genome_size} -untrimmed -pacbio-hifi ${corr_reads} -p ${params.region}_assembly -d canu_output
...
process asm_snv_call {
    ...
    """
    minimap2 -x map-ont -y --secondary=no -t ${task.cpu} -a ${contigs_fasta} ${region_fasta} | samtools sort - -o contig.bam && samtools index contig.bam
    ...
    if [ "\$(cut -f 1 asm_phase.gtf | uniq | wc -l)" -eq \$(cat asm_phase.gtf | wc -l) ]; then
        ...
        meth_phaser_post_processing -ib asm_tagged.bam -if methphaser_out/work -ov asm_meth_phase_temp.vcf -ob asm_meth_tagged_temp -vc asm_phase.vcf.gz -mc ${params.meth_min_reads} -vd ${params.meth_min_diff}
        ...
        python ${params.script_dir}/parse_methphaser_stats.py methphaser_out/work/*/*csv asm_meth_phase_stats.tsv --min_reads ${params.meth_min_reads} --min_percentage ${params.meth_min_diff}
    fi
    """
}
...
process asm_haplotype_specific {
    ...
    """
    canu maxMemory=24G maxThreads=${task.cpus} useGrid=false genomeSize=${params.genome_size} -untrimmed -pacbio-hifi ${hap_fasta} -p ${haplotype}_canu_output -d ${haplotype}_asm obtErrorRate=0.01
